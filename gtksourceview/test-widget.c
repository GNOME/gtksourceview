...
static void cb_move_cursor(GtkTextBuffer *buf,GtkTextIter *cursoriter, GtkTextMark *mark,gpointer data);
...
test_source(GtkSourceBuffer *buffer)
{
  ...
  if(!buffer) buffer = GTK_SOURCE_BUFFER(gtk_source_buffer_new(NULL));
  ...
  tag = gtk_pattern_tag_new("defs","^#[ \t]*\\(include\\|if\\|ifdef\\|ifndef\\|else\\|elif\\|define\\|endif\\|pragma\\)\\b");
  ...
  tag = gtk_pattern_tag_new("types","\\b\\(int\\|float\\|enum\\|bool\\|char\\|void\\|gint\\|"
  ...
  {
    GtkWidget *w;
    w=gtk_message_dialog_new(NULL,0,GTK_MESSAGE_ERROR,GTK_BUTTONS_OK, error->message);  
...
{
    gtk_source_view_set_show_line_pixmaps(GTK_SOURCE_VIEW(data), !GTK_SOURCE_VIEW(data)->show_line_pixmaps);
}
...
main (int argc, char *argv[])
{
  ...
  g_signal_connect_closure(G_OBJECT(buf), "mark_set", g_cclosure_new((GCallback)cb_move_cursor, label, NULL), TRUE);
