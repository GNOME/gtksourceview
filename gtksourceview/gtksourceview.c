...
static void gtk_source_view_real_populate_popup (GtkTextView *view, GtkMenu *menu);
...
static void check_line_change_insert_cb (GtkTextBuffer *buffer, GtkTextIter *iter,
					 const gchar *text, gint len, GtkTextView *view);
static void check_line_change_delete_cb (GtkTextBuffer *buffer, GtkTextIter *start,
					 GtkTextIter *end, GtkTextView *view);
static void check_line_change_delete_cb_after (GtkTextBuffer *buffer, GtkTextIter *start,
					       GtkTextIter *end, GtkTextView *view);

static void activate_cb (GtkWidget *menuitem, GtkTextView *text_view);
...
gtk_source_view_get_type (void)
{
	...
	if (our_type == 0) {
		...
		our_type =
			g_type_register_static (GTK_TYPE_TEXT_VIEW, "GtkSourceView", &our_info, 0);
	}
	return our_type;
}
...
gtk_source_view_class_init (GtkSourceViewClass *klass)
{
	...
	textview_class->populate_popup = gtk_source_view_real_populate_popup;
...
gtk_source_view_new_with_buffer (GtkSourceBuffer *buffer)
{
	...
	g_signal_connect_closure (G_OBJECT (buffer), "insert_text",
				  g_cclosure_new ((GCallback) check_line_change_insert_cb,
						  (gpointer) view, NULL), TRUE);
	g_signal_connect_closure (G_OBJECT (buffer), "delete_range",
				  g_cclosure_new ((GCallback) check_line_change_delete_cb,
						  (gpointer) view, NULL), FALSE);
	g_signal_connect_closure (G_OBJECT (buffer), "delete_range",
				  g_cclosure_new ((GCallback) check_line_change_delete_cb_after,
						  (gpointer) view, NULL), TRUE);
	return (view);
}
...
void
activate_cb (GtkWidget *menuitem, GtkTextView *text_view)
{
	const gchar *signal;
...
void
gtk_source_view_real_populate_popup (GtkTextView *text_view, GtkMenu *menu)
{
	GtkTextBuffer *buffer;
	GtkWidget *menuitem;
	...
	g_object_set_data (G_OBJECT (menuitem), "gtk-signal", (char *) "undo");
	gtk_signal_connect (GTK_OBJECT (menuitem), "activate", activate_cb, text_view);
	...
	g_object_set_data (G_OBJECT (menuitem), "gtk-signal", (char *) "redo");
	gtk_signal_connect (GTK_OBJECT (menuitem), "activate", activate_cb, text_view);
...
gtk_source_view_expose (GtkWidget *widget, GdkEventExpose *ev)
{
	...
	while (y < y2 && !gtk_text_iter_is_end (&iter)) {
		...
		gtk_text_view_buffer_to_window_coords (tw, GTK_TEXT_WINDOW_LEFT, 0, y, NULL, &pos);
		...
		if (view->show_line_numbers) {
			...
			gdk_draw_layout_with_colors (win, widget->style->fg_gc[widget->state],
						     text_width + GUTTER_PAD, pos, layout,
						     &GTK_WIDGET (tw)->style->fg[GTK_STATE_NORMAL],
						     &GTK_WIDGET (tw)->style->bg[GTK_STATE_NORMAL]);
			g_free (str);
		}
		if (view->show_line_pixmaps) {
			draw_line_markers (view, line_num, text_width + (GUTTER_PAD *2), pos);
		}
...
gtk_source_view_set_show_line_numbers (GtkSourceView *view, gboolean show)
{
	...
	if (!view->show_line_numbers && !view->show_line_pixmaps) {
		gtk_text_view_set_border_window_size (GTK_TEXT_VIEW (view), GTK_TEXT_WINDOW_LEFT,
						      0);
	} else {
...
gtk_source_view_set_tab_stop (GtkSourceView *view, gint tab_stop)
{
	...
	pango_tab_array_set_tab (tabs, 0, PANGO_TAB_LEFT,
				 get_tab_stop_width (GTK_WIDGET (view), tab_stop));
...
gboolean
foreach_character (gunichar ch, gpointer data)
{
...
recompute_gutter_width (GtkTextView *text_view)
{
	...
	old_width =
		text_view->left_window ? gtk_text_view_get_border_window_size (text_view,
									       GTK_TEXT_WINDOW_LEFT)
		: 0;
...
static void
check_line_change_insert_cb (GtkTextBuffer *buffer, GtkTextIter *iter, const gchar *text,
			     gint len, GtkTextView *view)
{
...
static void
check_line_change_delete_cb (GtkTextBuffer *buffer, GtkTextIter *start, GtkTextIter *end,
			     GtkTextView *view)
{
	gchar *slice;
...
static void
check_line_change_delete_cb_after (GtkTextBuffer *buffer, GtkTextIter *start, GtkTextIter *end,
				   GtkTextView *view)
{
	gchar *slice;
...
gboolean
gtk_source_view_add_pixbuf (GtkSourceView *view, const gchar *key, GdkPixbuf *pixbuf,
			    gboolean overwrite)
{
